...
boolean logger = true;
byte level = 100;
unsigned long relayModeTime = 20000;
//const byte id = 3;
...
void rTask(void *param) {
  Serial.print("receive task at core: ");
...
void sTask(void *param) {
  Serial.print("send task at core: ");
  Serial.println(xPortGetCoreID());
  int rD;
  rD = random(relayModeTime);
  vTaskDelay((rD) / portTICK_PERIOD_MS);
  myPacket(1);
  Serial.println("Sending my very own packet!!!!");
  sendPacket();
  vTaskDelete(NULL);
}
...
reset:
  ...
  if (packet_t.command == 1) {
    ...
    xTaskCreatePinnedToCore(rTask, "receive packet", 1024, NULL, 0, &rT, 1);
    xTaskCreatePinnedToCore(sTask, "send packet", 1024, NULL, 1, &sT, 1);
  ...
  if (packet_t.command == 2) {
    ...
    if (packet_t.level < level) {
      ...
      xTaskCreatePinnedToCore(rPeakTask, "receive peak g packet", 2048, NULL, 0, &rP, 1);
      xTaskCreatePinnedToCore(sPeakTask, "send peak g packet", 1024, NULL, 1, &sP, 1);
      delay(23000);
